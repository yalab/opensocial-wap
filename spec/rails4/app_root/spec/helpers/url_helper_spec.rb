...
describe OpensocialWap::Helpers::UrlHelper do
  ...
  describe "#link_to" do
    ...
    context NonOpensocialWapController do
      ...
      it "HTMLオプションが正しく追加されること" do
        ...
        link.should == %Q|<a class="user" href="/users/1">Alice</a>|
      end
    ...
    describe OpensocialWapController do
      ...
      it "HTMLオプションが正しく追加されること" do
        link = helper.link_to("Alice", users(:alice), :class=>"user")
        link.should == %Q|<a class="user" href="http://container.example.com/12345/?guid=ON&amp;url=http%3A%2F%2Ftest.host%2Fusers%2F1">Alice</a>|
      end
  ...
  describe "#extract_url_settings" do
    context OpensocialWapController do
      before do
        set_controller(OpensocialWapController.new)
      end

      it "url_settings オプションは Proc を受け取れる" do
        link = helper.link_to("Index", { :controller => "users", :action => "index" }, :url_settings => proc{|context|{:format => :local}})
        link.should == %Q|<a href="http://test.host/users">Index</a>|
      end
    end
  end
end
